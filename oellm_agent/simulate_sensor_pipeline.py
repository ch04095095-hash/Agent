...
def base_state(step: int) -> SensorState:
    ...
    # 与 thresholds.json 对齐：speed_warn_high_kmh=13、speed_alarm_high_kmh=14
...
def classify_warning(s: SensorState) -> Tuple[str, str]:
    ...
    # 稳态但存在轻微工况压力
    if s.speed_kmh < 1.0 and s.payload_tons > 170:
...
def inject_llm_case(s: SensorState, case: str, phase: float) -> None:
    ...
    if case == "coolant_fast_rise":
        ...
        s.speed_kmh = clamp(12 + phase * 11 + random.gauss(0, 0.15), 0, 26)
...
def encode_frames(s: SensorState) -> List[Tuple[int, List[int]]]:
    # 严格对齐 can_decoder.py 的 LuTech 协议
    ...
    brake_p = max(0, min(200, int(round(s.brake_pressure_bar))))
    travel_p = max(0, min(400, int(round(s.travel_pressure_bar))))
    system_p = max(0, min(200, int(round(s.system_pressure_bar))))
    clamp_p = max(0, min(200, int(round(s.clamp_pressure_bar))))

    # 2.3 甲烷浓度、车速、转速、角度
    methane_raw = max(0, min(800, int(round(s.methane_pctlel * 10.0 + 400.0))))
    speed_raw = max(0, min(251, int(round(s.speed_kmh * 10.0))))
    ...
    return [
        ...
        (0x18F182A0, [brake_p, travel_p, system_p, clamp_p, 0, 0, 0, 0]),
